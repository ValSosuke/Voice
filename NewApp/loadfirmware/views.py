        ...
        with open(patth_dir+fs.name, 'rb+',0) as destination1, closing(mmap.mmap(destination1.fileno(), 0, access=mmap.ACCESS_READ)) as s:
            ...
            if (struct_firmware.s_major==4):
                if (struct_firmware.compression==2):
                    ...
                    commands.getoutput("sudo ./Squashfs/sasquatch-master/squashfs4.3/squashfs-tools/sasquatch " + "-f -d "+ patth_dir+"/FS" +str(i) +" "+patth_dir+fs.name+'.squash' +' \n ' +"sudo ./Squashfs/sasquatch-master/squashfs4.3/squashfs-tools/sasquatch " + "-f -d "+ patth_dir+"/FS" +str(i) +" "+patth_dir+fs.name+'.squash')
                    commands.getoutput("sudo ./Squashfs/sasquatch-master/squashfs4.3/squashfs-tools/sasquatch " + "-f -d "+ patth_dir+"/FS" +str(i) +" "+patth_dir+fs.name+'.squash' +' \n ' +"sudo ./Squashfs/sasquatch-master/squashfs4.3/squashfs-tools/sasquatch " + "-f -d "+ patth_dir+"/FS" +str(i) +" "+patth_dir+fs.name+'.squash')               
...
#--------------------------------------------------------------------
    ...
    print len(all_expansion1)
    print len(all_expansion2)
    ...
    for index in range(len(all_expansion1)):
        ...
        dif_files2.append(dif_files_buf)# Есть в FS1, но нет в FS0
        dif_files_buf=[]
    ...
    for t in foold_all[0]: # список файлов и папок для 1 ФС
        for filename in t[2]:
            ...
            if ((m.file(t[0]+"/"+filename))[0:20].find('link') == -1) and ((m.file(t[0]+"/"+filename))[0:20].find(r'special') == -1):
                ...
                elif ((m.file(t[0]+"/"+filename)[0:6]).find('HTML')>=0) or (filename.find('.html')>=0) or (filename.find('.htm')>=0):
    ...
    for t in foold_all[1]: # список файлов и папок для 1 ФС
        for filename in t[2]:
            #if(filename=="webs_upgrade.sh"): print "hello" 
            if ((m.file(t[0]+"/"+filename))[0:20].find('link') == -1) and ((m.file(t[0]+"/"+filename))[0:20].find(r'special') == -1):
                #if(filename=="webs_upgrade.sh"): print "hello"
                ...
                if fll==1:
                    ...
                    #break
                    continue
                #if (filename == 'js_list'): print "fs2"
                if ((m.file(t[0]+"/"+filename)[0:5]).find('ELF')>=0):
                    shutil.copy2(t[0]+"/"+filename, patth_dir+"/bin/FS1")
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print "fs3"
                elif ((m.file(t[0]+"/"+filename)[0:5]).find('HTML')>=0) or (filename.find('.html')>=0) or (filename.find('.htm')>=0):
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print "fs4"
                    shutil.copy2(t[0]+"/"+filename, patth_dir+"/html/FS1")
                elif (filename.find('.js')>=0):
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print "fs5"
                    shutil.copy2(t[0]+"/"+filename, patth_dir+"/script/FS1")
                elif (filename.find('.sh')>=0):
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print "fs6"
                    shutil.copy2(t[0]+"/"+filename, patth_dir+"/script/FS1")
                elif (filename.find('.conf')>=0):
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print "fs7"
                    shutil.copy2(t[0]+"/"+filename, patth_dir+"/configure/FS1")
                else:
                    if (filename == 'WzdAccessCtrlSchedAddHelpRpm.htm'): print shutil.copy2(t[0]+"/"+filename, patth_dir+"/other/FS1")
...
def upload_file(request):
    ...
    if request.method == 'POST':
        form = UploadFileForm(request.POST, request.FILES, auto_id = False)
        print form['Select_functions']
        #form = UploadFileForm(request.POST, request.FILES, initial={'Select_functions':[]})
        
        if form.is_valid():
            #print len(form.cleaned_data['Select_functions'])
            if len(form.cleaned_data['Select_functions']) == 0:
                print len(form.cleaned_data['Select_functions'])
                return Http404("Poll does not exist")
            form.cleaned_data.get('Select_functions')
            ...
            flagflag = 0
            for i in tablenamefw:
                if (i.name_firmawre_1==request.FILES.getlist('file')[0].name and i.name_firmawre_2==request.FILES.getlist('file1')[0].name) or (i.name_firmawre_2==request.FILES.getlist('file')[0].name and i.name_firmawre_1==request.FILES.getlist('file1')[0].name):
                    flagflag = 1
                    idd = i.id_name
                    i.delete()
                    break
            ...
            if flagflag == 0:
                tablenamefw = TableNameFW()
                tablenamefw.name_firmawre_1=request.FILES.getlist('file')[0].name
                tablenamefw.name_firmawre_2=request.FILES.getlist('file1')[0].name
                tablenamefw.save()
                ...
                for i in list_script:
                    if i[0]!='':
                        tablescriptsfw = TableScriptsFW()
                        tablescriptsfw.id_name = tablenamefw.id_name
                        tablescriptsfw.script_name = i[0] 
                        tablescriptsfw.data_script = i[1].decode('cp1252', errors='ignore')
                        tablescriptsfw.save()

                for i in list_html:
                    if i[0]!='':    
                        #print i[0]
                        tablehtmlfw = TableHtmlFW()
                
                        tablehtmlfw.id_name = tablenamefw.id_name
                        tablehtmlfw.html_name = i[0] 
                        tablehtmlfw.data_html = i[1].decode('cp1252', errors='ignore')
                        tablehtmlfw.save()

                for i in list_configure:
                    if i[0]!='':
                        tableconfigurefw = TableConfigureFW()
                        tableconfigurefw.id_name = tablenamefw.id_name
                        tableconfigurefw.configure_name = i[0] 
                        tableconfigurefw.data_configure = i[1].decode('cp1252', errors='ignore')
                        tableconfigurefw.save()

                for i in list_other:
                    if i[0]!='':
                        tableotherfw = TableOtherFW()
                        tableotherfw.id_name = tablenamefw.id_name
                        tableotherfw.other_name = i[0] 
                        tableotherfw.data_other = i[1].decode('cp1252', errors='ignore')
                        tableotherfw.save()
                #for i in list_all_binary:
                #    if i[0]!='':
                #        tablebinaryfw = TableBinaryFW()
                #        for j in i[1]:
                #            tablebinaryfw.id_name = tablenamefw.id_name
                #            tablebinaryfw.valn_function_name = i[0] 
                #        
                #            tablebinaryfw.file_name = j[0]
                #            tablebinaryfw.data_binary = j[1]
                #            tablebinaryfw.save()
                tablebinaryfw = TableBinaryFW()
                        
                tablebinaryfw.id_name = tablenamefw.id_name
                            #tablebinaryfw.file_name = i[0] 
                        
                            #tablebinaryfw.valn_function_name = j[0]
                            #tablebinaryfw.data_binary = ' '.join(j[1])
                            #tablebinaryfw.data_binary=pickle.dumps(j[1])
                tablebinaryfw.data_binary=pickle.dumps(list_all_binary)
                tablebinaryfw.save()


            else:
                
                TableScriptsFW.objects.filter(id_name=idd).delete()
                TableHtmlFW.objects.filter(id_name=idd).delete()
                TableOtherFW.objects.filter(id_name=idd).delete()
                TableConfigureFW.objects.filter(id_name=idd).delete()
                TableBinaryFW.objects.filter(id_name=idd).delete()
                tablenamefw = TableNameFW()
                tablenamefw.name_firmawre_1=request.FILES.getlist('file')[0].name
                tablenamefw.name_firmawre_2=request.FILES.getlist('file1')[0].name
                tablenamefw.save()
           ...
           # print form.cleaned_data['Select_functions']
